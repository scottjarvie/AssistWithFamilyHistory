...
| `/api/operations/conflicts` | POST | Settle a source-backed fact flagged as conflicting: record which reading the person believed and why | Uses `getVaultAccessContext()` and Convex owner checks | **Never an agent route.** A chosen AI may flag a conflict and propose an answer on the `conflict_resolution` research task; deciding which record to believe is the person's judgment call (AWF-0046) | The reading decided against is kept, never deleted; the person's canonical fact is not rewritten; a reason of at least 20 characters is required and stored as durable research log text |
