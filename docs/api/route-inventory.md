...
| `/api/keys/[keyId]` | DELETE | Revoke one of the owner's API keys (idempotent) | Uses `getVaultAccessContext()` and Convex owner check | Internal-only (human-managed) | Credential revocation; owner-guarded in the Convex mutation |
