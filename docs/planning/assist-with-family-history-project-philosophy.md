...
>
> **Repository evidence revision:** `AWF-WO-011 deployed; person-approved grants, scope enforcement, namespaced tools with aliases, batch writes, evidence gating including real media bytes, and the connection centre are Current in production; provider dynamic client registration is available and CIMD is deferred; a real AI client's whole lifecycle, and therefore any named client, remain Partial`
>
