...
| MCP and AI setup paths | **Current in source and tests: person-approved grants, enforced scopes, namespaced tools, batch writes, evidence gating, and the connection centre; Partial: deployed proof, provider CIMD/DCR, media byte delivery, and any named client** | Signing in no longer behaves like blanket authority. Every `/mcp` request resolves one durable product grant the person approved, and fails closed when it is absent, pending, expired, or revoked. Six scopes are the whole ceiling; fourteen namespaced tools whose names all begin `family_history_` carry twelve pre-namespace aliases on the same handlers so nothing that already connected breaks. Because the transport is stateless and the grant is re-resolved per request, turning a connection off denies its very next call — no token has to expire first. `family_history_save_records` saves a whole source's worth of records in one call with per-item results; `family_history_get_evidence` delivers reviewed, AI-allowed items as content blocks and never hands a model a storage link. An unknown tool, an ungranted tool, another owner's record, and an invented id all return the same refusal. `/app/settings/ai` lets a person approve, narrow, and revoke, with its consent screen generated from the same catalog the edge enforces, and `/ai` and `/ai.txt` generated from it too. **This is proof from source and tests, not from a deployment.** Nothing here has run against the live site, `/app/settings/ai` has not been opened in a browser, live provider metadata on 2026-08-16 advertised neither Client ID Metadata Documents nor a registration endpoint, media stored only as a file reference returns `BYTES_NOT_AVAILABLE` rather than bytes, and no client is named as compatible. AWF-0043 owns the live eleven-point lifecycle that would change any of that |
