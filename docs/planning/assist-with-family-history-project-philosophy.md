...
>
> **Product document version:** 1.10.0
>
> **Philosophy date:** 2026-09-03
>
> **Capability evidence last verified:** 2026-09-03
>
> **Repository evidence revision:** `AWF-WO-011 and the image-only portion of AWF-WO-012 are deployed: seven person-approved permissions, sixteen namespaced tools plus twelve compatibility aliases, batch writes, reviewed evidence reads, direct private image upload, and the connection centre are Current in production; provider DCR and the isolated Backblaze storage boundary are separately proven; a joined real-client upload-to-review-to-read lifecycle, and therefore any named client, remain Partial`
>
...
|---|---|---|
| Person / Assist workspace / your AI split | **Current / verified philosophy, public setup, and version 2.0.0 production home** | The enduring split below makes the person authoritative, Family History durable, and “your AI” user-chosen. `/ai` teaches that split in Family History's archival voice; the live version 2.0.0 home uses “Assist your AI, so it can assist you with family history,” links the connection path early, and carries an eight-question capability and authority FAQ. AWF-WO-007 records public, signed-in, and disposable-client proof |
...
| Queue authority | **Current / verified Queue-command foundation; Coming soon external identity proof** | Queue commands enforce bounded leases, exact actor/item operations, and the family rule that attached context grants no domain mutation authority. A Queue item never silently expands authority. Product grants must explicitly bind actor/AI, scope, permitted data, operation or external-action category, approval, and expiry/revocation. Missing authority returns to **Needs you** with the smallest exact question. Where a complete grant already exists, the Queue may give that chosen AI a recorded external-action intent and selected context, then attribute success, denial, failure, or return; Family History itself never performs the outside-world action. Incoming chosen-AI credentials now resolve to a person-approved product grant, and `family_history:queue:read` / `family_history:queue:work` derive the Queue principal's scopes instead of the previous hard-coded full set. That binding is proved in source and tests, not yet against the deployed site |
...
| MCP and AI setup paths | **Current in production: person-approved grants, enforced scopes, namespaced tools, batch writes, reviewed evidence reads, direct private image upload, and the connection centre; Partial: one real AI client's joined lifecycle, and therefore any named client; Deferred: Client ID Metadata Documents** | Signing in grants no Family History authority by itself. Every `/mcp` request resolves one durable product grant the person approved and fails closed when it is absent, pending, expired, or revoked. Seven permissions are the whole ordinary ceiling; sixteen namespaced tools whose names begin `family_history_` carry twelve pre-namespace aliases on the same handlers. No current permission is silently defaulted or separately raised: the person selects each one, while publication, deletion, identity merge, sharing changes, external-provider actions, billing, export, and archive/restore remain unissuable. `family_history_save_records` saves a batch with per-item results and replay receipts; `family_history_get_evidence` returns reviewed, AI-allowed bytes without exposing a storage link. The two evidence-upload tools accept JPEG, PNG, and WebP through a short-lived same-origin capability into private Backblaze storage, but every upload remains private, unreviewed, rights-unknown, and AI-disallowed until the person reviews it. The isolated provider proof verified exact bytes, hash, reciprocal credential denial, cleanup, and zero residue; it does not prove a real client completed upload, person review, later read, correction, revoke, reconnect, and cleanup as one lifecycle. `/app/settings/ai`, `/ai`, and `/ai.txt` derive consent and public truth from the catalog, and both protected-resource discovery spellings resolve directly without redirect. Provider DCR is live, but the application cannot infer or inventory provider-registration provenance from an opaque `client_id`; the local CIMD validator is strict and presently unused. No named AI client is claimed compatible until that exact client completes the joined lifecycle. |
...
| Launch stage | **Current / verified soft-launch production on the canonical Assist With Family History address** | The normal domain, branded sign-in, isolated empty workspace/Queue, public setup/discovery, fail-closed anonymous boundary, and disposable-client OAuth/MCP workflow are proven. This is not a big/public launch: normal fresh-device email verification, broad client availability, reconnect, immediate JWT revocation, catalog listing, and real-family usage remain separate proof or product gates |
...
| Scoped authority | Seven MCP product permissions, durable chosen-AI grants, API-key mint/list/suspend/revoke primitives, owner checks, story roles, guarded publish actions, selected review/audit events, and bounded Queue command scopes | **Current / verified chosen-AI grant foundation; Partial wider authority model.** OAuth sign-in grants no product authority. A fresh owner-scoped grant binds client, permissions, record boundary, consent snapshot, expiry/revocation, and activity, while Queue commands grant no extra domain mutation. No current MCP permission is separately raised; reserved destructive, public, sharing, identity, provider, billing, export, and archive actions are unissuable. Incoming API-key resolution and a general external-action category/result contract remain unfinished; self-asserted story headers are not a public authorization model |
| API and AI connections | Internal app APIs, API-key lifecycle management, Queue scope presets, `agentActivity`, capability manifest, story OpenAPI skeleton, `/api/capabilities`, API Center source, `/llms.txt`, plus `/mcp`, `/ai`, and `/ai.txt` | **Current / verified stateless production foundation; Partial complete Bring Your AI connection.** Sixteen namespaced workflow tools plus twelve compatibility aliases resolve the owner from an exact-issuer Clerk OAuth access token and a fresh durable person-approved grant. Seven product permissions are enforced independently of provider identity scopes; OAuth alone grants none. Reads stay bounded, writes carry receipts and replay protection, reviewed evidence bytes never expose provider URLs, and revocation denies the next request. Image upload is current through a private, checksum-bound Backblaze path; broader media classes and the joined real-client upload-to-review-to-read lifecycle remain Partial. Provider DCR is live and CIMD is deferred, but the application does not currently receive trustworthy DCR provenance or maintain provider registration inventory. The machine manifest now mirrors the exact tool and authority tiers; request/response/error schema ownership and any future public API/SDK/CLI parity remain proposed contract work. No named client is compatible until its exact lifecycle proof. |
...
- universal “works with any AI” compatibility, a named compatible client,
  Client ID Metadata Document onboarding, application-observed provider DCR
  provenance/inventory, production-authenticated refresh/reconnect, or a joined
  real-client upload/review/read/revoke/cleanup lifecycle;
...
- **1.10.0 · 2026-09-03** — corrected the chosen-AI capability record to the
  deployed seven-permission, sixteen-tool catalog and the private image-upload
  work. Made the authority tiers explicit: OAuth grants nothing automatically,
  every ordinary permission is person-selected, no separate raised tier exists,
  and reserved destructive/public/external actions remain unissuable. Recorded
  the exact boundary between live provider DCR and application-visible client
  provenance, the two direct discovery spellings, the isolated Backblaze byte/
  denial/cleanup proof, and the still-missing joined real-client lifecycle. No
  named client or generic public API is promoted by this evidence correction.
- **1.9.0 · 2026-08-16** — moved the chosen-AI connection from *planned* to
  *built in source and tests*, without moving it to *proved in production*.
  Person-approved product grants, the six-scope ceiling enforced on every
  request, record boundaries, immediate revocation, namespaced tools with
  compatibility aliases, the batch record save, protected evidence delivery, and
  the `/app/settings/ai` connection centre are now Current in source and covered
  by tests. Deployed proof, provider Client ID Metadata Document or registration
  support, byte delivery for media stored only as a reference, and any named
  client remain Partial and are named as such on `/ai` itself. Writing the
  lifecycle harness exposed two real interoperability defects — hand-written
  wire responses missing protocol revision 2026-07-28's required `resultType`,
  and a connection label read from the tool-name header — both now fixed and
  guarded. AWF-0043 owns the live eleven-point lifecycle.
