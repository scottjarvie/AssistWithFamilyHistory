...
>
> **Product document version:** 1.9.0
>
> **Philosophy date:** 2026-08-16
>
> **Capability evidence last verified:** 2026-08-17
>
> **Repository evidence revision:** `AWF-WO-011 deployed; person-approved grants, scope enforcement, namespaced tools with aliases, batch writes, evidence gating including real media bytes, advertised scopes_supported, and the connection centre are Current in production; provider dynamic client registration is available and CIMD is deferred; a real AI client's whole lifecycle, and therefore any named client, remain Partial`
>
...
| MCP and AI setup paths | **Current in production: person-approved grants, enforced scopes, namespaced tools, batch writes, evidence gating including real media bytes, an advertised `scopes_supported` vocabulary, and the connection centre; Partial: a real AI client's whole lifecycle, and therefore any named client; Deferred: Client ID Metadata Documents** | Signing in no longer behaves like blanket authority. Every `/mcp` request resolves one durable product grant the person approved, and fails closed when it is absent, pending, expired, or revoked. Six scopes are the whole ceiling; fourteen namespaced tools whose names all begin `family_history_` carry twelve pre-namespace aliases on the same handlers so nothing that already connected breaks. Because the transport is stateless and the grant is re-resolved per request, turning a connection off denies its very next call — no token has to expire first. `family_history_save_records` saves a whole source's worth of records in one call with per-item results; `family_history_get_evidence` delivers reviewed, AI-allowed items as content blocks and never hands a model a storage link. Media the vault holds is delivered as real bytes: a person can upload a scan, photograph, record PDF, transcript, or recording, it lands in private storage no link can reach, and the connection reads the object directly so an AI sees the census page itself rather than its title. Uploading is not the same act as allowing AI use — every file arrives private, unreviewed, and AI-disallowed, and replacing bytes resets review — and a row that still holds only a reference elsewhere stays honestly `BYTES_NOT_AVAILABLE`. An unknown tool, an ungranted tool, another owner's record, and an invented id all return the same refusal. `/app/settings/ai` lets a person approve, narrow, and revoke, with its consent screen generated from the same catalog the edge enforces, and `/ai` and `/ai.txt` generated from it too. **This is deployed, and it is proved by source and tests — but no client lifecycle has been run against it.** No AI client has yet gone all the way through here: discover, register, consent, list tools, read, write, fetch evidence, revoke, be denied. `/app/settings/ai` is live but has not been exercised in a browser, no person has uploaded a file through a real browser, and no client is named as compatible. Deployed is a fact; proved by a client is not. Live provider metadata re-probed on 2026-08-17 does now advertise a registration endpoint, so dynamic client registration is the approved soft-launch onboarding path and Client ID Metadata Documents are deferred until the provider offers them. AWF-0043 owns the live eleven-point lifecycle that would change any of that |
