...
type PageType = "sources" | "memories" | "person";
...
function getPageInfo(): PageInfo {
  ...
  const pageType: PageType = url.includes("/memories")
    ? "memories"
...
async function extractSourceData(
  ...
  const linkEl = container.querySelector("a[href*='ark:'], a[href*='/record/']") as HTMLAnchorElement | null;
  ...
  let showButton: Element | null = null;
...
function extractMemories(pageInfo: PageInfo): MemoryData[] {
...
function buildCapturePackage(
  pageInfo: PageInfo,
  items: { sources: SourceData[]; memories: MemoryData[] },
  startTime: number,
  pacing: PacingConfig,
): CapturePackage {
  ...
  return {
    ...
    sources: items.sources.map((source) => ({
      ...
      placeMentions: source.indexed.fields
        ...
        .map((field) => ({ name: field.value, role: field.label })),
      outboundUrls: [source.webPageUrl].filter((value): value is string => Boolean(value)),
    ...
    diagnostics: {
      ...
      failedExpansions: items.sources.filter(
        (source) => source.expansionAttempts > 0 && !source.expansionSucceeded,
      ).length,
      warnings: [],
      errors: [],
    },
...
function inferSourceType(source: SourceData): "record" | "memory" | "story" | "photo" | "other" {
