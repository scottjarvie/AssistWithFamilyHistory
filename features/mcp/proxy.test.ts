...
describe("branded Family History MCP proxy", () => {
  test("reconstructs the Convex site URL and preserves metadata headers", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: string | URL | Request) => {
      ...
      return Response.json({ resource: "https://assistwithfamilyhistory.com/mcp" }, {
        headers: { "cache-control": "public, max-age=300", "access-control-allow-origin": "*" },
      });
    }));
    const response = await getMetadata(new Request("https://assistwithfamilyhistory.com/.well-known/oauth-protected-resource/mcp"));
    expect(response.status).toBe(200);
    expect(response.headers.get("cache-control")).toBe("public, max-age=300");
    await expect(response.json()).resolves.toMatchObject({ resource: "https://assistwithfamilyhistory.com/mcp" });
  });
  ...
  test("preserves the anonymous challenge and rejects an oversized body before forwarding", async () => {
    ...
    const fetchMock = vi.fn(async () => Response.json({ error: "invalid_token" }, {
      status: 401,
      headers: { "www-authenticate": 'Bearer resource_metadata="https://assistwithfamilyhistory.com/.well-known/oauth-protected-resource/mcp"' },
    }));
    vi.stubGlobal("fetch", fetchMock);
    const challenge = await proxyFamilyHistoryMcpRequest(new Request("https://assistwithfamilyhistory.com/mcp", {
      method: "POST", headers: { "content-type": "application/json" }, body: "{}",
    }), "/mcp");
    expect(challenge.status).toBe(401);
    expect(challenge.headers.get("www-authenticate")).toContain("resource_metadata=");

    const oversized = await proxyFamilyHistoryMcpRequest(new Request("https://assistwithfamilyhistory.com/mcp", {
  ...
  test("returns a live upstream stream without forwarding stale framing headers", async () => {
    ...
    const responsePromise = proxyFamilyHistoryMcpRequest(
      new Request("https://assistwithfamilyhistory.com/.well-known/oauth-protected-resource/mcp"),
